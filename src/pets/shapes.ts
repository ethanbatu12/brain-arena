...
export const PET_SHAPES: Record<PetSpecies | "seasonFox" | "seasonDragon" | "seasonPhoenix" | "seasonLion", PetShape> = {
  ...
  // Season Pass-exclusive pets — generic archetypes reused across themes (only the color/name differ per season).
...
export function shapeFor(key: string): PetShape {
  return (PET_SHAPES as Record<string, PetShape>)[key] ?? DEFAULT_SHAPE;
}
